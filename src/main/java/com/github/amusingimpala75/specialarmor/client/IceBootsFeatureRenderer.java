...
public class IceBootsFeatureRenderer<T extends LivingEntity, A extends BipedEntityModel<T>> extends FeatureRenderer<T, A> {
    private final A bodyModel;

    public IceBootsFeatureRenderer(FeatureRendererContext<T, A> context, A bodyModel) {
        super(context);
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        ...
        if (itemStack.getItem() instanceof IceBoots) {
            ...
            VertexConsumer vertexConsumer = ItemRenderer.getArmorGlintConsumer(vertexConsumerProvider, RenderLayer.getArmorCutoutNoCull(SpecialArmor.id("textures/models/armor/ice_boots.png")), false, bl2);
